  ...
  if (state.ok) {
    return (
      <div className="rise flex flex-col items-start gap-4 rounded-2xl border border-lime/30 bg-lime/[0.05] p-6 sm:p-8">
        <span className="font-mono text-[12.5px] tracking-[0.12em] text-lime uppercase">Received</span>
        <p className="text-[18px] text-fg">{state.message}</p>
      </div>
    );
  }
  ...
  return (
    <form action={action} noValidate className="flex flex-col gap-5">
      ...
      {!state.ok && state.message && (
        <p role="alert" className="font-mono text-[13px] text-red">{state.message}</p>
      )}
...
const input =
  "w-full rounded-lg border border-line bg-surface-2 px-3.5 py-2.5 text-[16px] text-fg transition-colors placeholder:text-fg-3 hover:border-line-strong focus:border-brand focus:outline-none";
...
function Field({
  ...
  return (
    <div className="flex flex-col gap-1.5">
      <label htmlFor={htmlFor} className="flex items-baseline justify-between text-[15px] font-medium text-fg-2">
        {label}
        {hint && <span className="font-mono text-[12.5px] text-fg-3">{hint}</span>}
      </label>
      {children}
      {error && <p className="font-mono text-[13px] text-red">{error}</p>}
    </div>
  );
}
