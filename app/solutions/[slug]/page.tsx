    ...
    <PageShell>
      ...
      <Section index="01" eyebrow="Problem and outcome" title="What changes">
        <Reveal variant="up" className="grid gap-px overflow-hidden rounded-2xl border border-line bg-line md:grid-cols-2">
          <div className="stagger flex flex-col gap-3 bg-ink p-6 sm:p-8" style={{ "--i": 0 } as React.CSSProperties}>
            <span className="font-mono text-[11px] tracking-[0.12em] text-paper-3 uppercase">Today</span>
            <p className="text-[15.5px] leading-relaxed text-paper-2">{s.problem}</p>
          </div>
          <div className="stagger flex flex-col gap-3 bg-ink p-6 sm:p-8" style={{ "--i": 1 } as React.CSSProperties}>
            <span className="font-mono text-[11px] tracking-[0.12em] text-brand uppercase">With KonetCloud</span>
            <p className="text-[15.5px] leading-relaxed text-paper">{s.outcome}</p>
          </div>
        </Reveal>
      </Section>
      ...
      <Section index="04" eyebrow="Considerations" title="Migration, implementation and operations" description="Benefits are stated only where evidence exists; we would rather show you than tell you.">
        <Reveal as="ul" variant="up" className="grid gap-3 md:grid-cols-3">
          {s.considerations.map((c, i) => (
            <li key={c} className="stagger rounded-xl border border-line bg-ink-2 p-5 text-[14px] leading-relaxed text-paper-2" style={{ "--i": i } as React.CSSProperties}>
              {c}
            </li>
          ))}
        </Reveal>
