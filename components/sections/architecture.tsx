...
export function ArchitectureDiagram({ className }: { className?: string }) {
  return (
    <figure
      className={cn(
        "rounded-2xl border border-line bg-ink-2 p-4 sm:p-6",
        className,
      )}
    >
      <figcaption className="flex flex-wrap items-center justify-between gap-2 font-mono text-[11px] tracking-[0.12em] text-paper-3 uppercase">
      ...
      <div className="mt-5 flex flex-col gap-3">
        ...
        <div className="relative">
          ...
          <div className="relative">
            ...
            <ol className="grid gap-3 md:grid-cols-3">
              {availabilityZones.map((z, i) => (
                <li
                  key={z.id}
                  className="stagger relative flex flex-col gap-4 rounded-xl border border-line bg-ink p-4"
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      ...
                      <h4 className="mt-1 text-[15px] font-semibold tracking-[-0.01em] text-paper">
                        {z.name}
                      </h4>
                    </div>
                  ...
                  <ul className="flex flex-wrap gap-1.5">
                    {capabilities.map((c) => (
                      <li
                        key={c}
                        className="rounded-md border border-line bg-ink-2 px-2 py-1 text-[11.5px] text-paper-2"
                      >
                        {c}
                      </li>
                    ))}
                  </ul>
                  <p className="font-mono text-[10.5px] tracking-[0.1em] text-paper-3 uppercase">
                    Key services
                  </p>
                </li>
              ))}
          ...
          <p className="mt-3 text-center font-mono text-[11px] text-paper-3 md:hidden">
  ...
  return (
    <div
      className={cn(
        ...
        accent ? "border-brand/30 bg-brand/[0.05]" : "border-line bg-ink",
      )}
    >
      <span className="text-[13px] font-semibold tracking-[-0.01em] text-paper">
        {label}
      </span>
      <ul className="flex flex-wrap gap-x-4 gap-y-1 font-mono text-[11px] text-paper-3">
        {items.map((it) => (
          <li key={it}>{it}</li>
        ))}
      </ul>
